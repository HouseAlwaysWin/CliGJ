//! Shell profile rows for top-right terminal picker (`name` + startup command).
...
pub(crate) fn default_shell_profiles() -> Vec<(String, String)> {
    vec![
        (
...
pub(crate) fn load_from_config(cfg: &AppConfig) -> (Vec<(String, String)>, bool) {
    ...
    for (name, cmd) in &defaults {
        if let Some((_, c)) = profiles.iter().find(|(n, _)| n == name) {
            let (norm, fixed) = normalize_shell_profile_command(c);
            if fixed {
                changed = true;
            }
            out.push((name.clone(), norm));
        } else {
...
pub(crate) fn is_reserved_shell_profile(name: &str) -> bool {
    default_shell_profiles().iter().any(|(n, _)| n == name)
}
...
pub(crate) fn default_shell_profile_name(gs: &GuiState) -> String {
    if let Some((name, _)) = gs.shell_profiles.first() {
        return name.clone();
    }
    "Command Prompt".to_string()
}
