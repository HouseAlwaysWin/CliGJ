...
pub(super) fn connect_prompt_and_picker(
    app: &AppWindow,
    state: Rc<RefCell<GuiState>>,
    history_window: Rc<TerminalHistoryWindow>,
) {
    prompt_input_callbacks::connect(app, Rc::clone(&state), Rc::clone(&history_window));
    interactive_manage_callbacks::connect(app, Rc::clone(&state));
    shell_manage_callbacks::connect(app, Rc::clone(&state), history_window);
}
